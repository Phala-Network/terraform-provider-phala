...
- `phala_app_instance.env` now updates **in place** instead of forcing replacement. Changing a slot's env encrypts the new values against the app KMS key and `PATCH`es `/cvms/{uuid}/envs` on that slot, preserving its `vm_uuid` — mirroring `phala_app.env`. Previously any env edit to a managed slot destroyed and recreated its CVM, which churned every non-bootstrap slot of a members-mode MIG on a single env change (surfaced by the large-scale e2e stress test). Per-instance env remains rejected on adopted (`managed = false`) slots, as before. Live-verified on real cloud: env edit applies with `vm_uuid` preserved and a clean follow-up plan.
