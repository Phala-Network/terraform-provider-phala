...
subcategory: ""
description: |-
  Runs Phala Cloud app provision/preflight and stores the resulting compose hash as Terraform state. No remote CVM is created.
---

# phala_app_preflight (Resource)

Runs Phala Cloud app provision/preflight and stores the resulting compose hash
as Terraform state. No remote CVM is created.

Use the resource form when another Terraform resource depends on the preflight
result. Unlike the data source form, destroy refresh does not re-run preflight;
Terraform reads the stored artifact and then removes it from state.

## Example Usage

```terraform
resource "phala_app_preflight" "worker_1" {
  name      = "demo-worker-1"
  size      = "tdx.small"
  region    = "US-WEST-1"
  disk_size = 20
  ...
  env_keys       = sort(keys(local.worker_env))
...
output "worker_compose_hash" {
  value = phala_app_preflight.worker_1.compose_hash
}
```
